...
class TestEnsureStorageDir:
    ...
    def test_ensure_storage_dir_creates_directory(
        self, temp_storage_dir, mock_settings
    ):
    ...
    def test_ensure_storage_dir_with_no_store_path(
        self, temp_storage_dir, mock_settings
    ):
...
class TestCaptureRequestToFile:
    ...
    @pytest.mark.asyncio
    async def test_capture_request_to_file_creates_json(
        self, mock_request, temp_storage_dir, mock_settings
    ):
    ...
    @pytest.mark.asyncio
    async def test_capture_request_to_file_stores_file(
        self, mock_request, temp_storage_dir, mock_settings
    ):
    ...
    @pytest.mark.asyncio
    async def test_capture_request_with_json_body(
        self, mock_request, temp_storage_dir, mock_settings
    ):
    ...
    @pytest.mark.asyncio
    async def test_capture_request_with_non_utf8_body(
        self, temp_storage_dir, mock_settings
    ):
...
class TestForwardToUpstream:
    ...
    @pytest.mark.asyncio
    async def test_forward_to_upstream_success(
        self, mock_request, mock_settings, temp_storage_dir
    ):
        ...
        try:
            ...
            response = await forward_to_upstream(
                mock_request, captured_req, "test-api-key"
            )
    ...
    @pytest.mark.asyncio
    async def test_forward_to_upstream_with_api_key(
        self, mock_request, mock_settings, temp_storage_dir
    ):
    ...
    @pytest.mark.asyncio
    async def test_forward_to_upstream_without_config(
        self, temp_storage_dir, mock_settings
    ):
    ...
    @pytest.mark.asyncio
    async def test_forward_to_upstream_with_upstream_error(
        self, temp_storage_dir, mock_settings
    ):
        ...
        try:
            with patch(
                ...
                mock_instance.__aenter__ = MagicMock(
                    return_value=asyncio_coroutine(mock_instance)
                )
                mock_instance.__aexit__ = MagicMock(
                    return_value=asyncio_coroutine(None)
                )
    ...
    async def test_proxy_all_methods(self, app, client):
        ...
        for method in methods:
            with patch(
                ...
                mock_instance = MagicMock()
                mock_instance.request = MagicMock(
                    return_value=asyncio_coroutine(Response(status_code=200))
                )
                mock_instance.__aenter__ = MagicMock(
                    return_value=asyncio_coroutine(mock_instance)
                )
                ...
                response = client.request(
                    method, f"/proxy/test", headers={"x-auth-token": str(token)}
                )
